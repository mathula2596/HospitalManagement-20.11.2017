...
<form action="<?php $_SERVER['PHP_SELF'] ?>" name="myform" method="post" enctype="multipart/form-data">
...
<td><input type="text" name="no" size="30" value="<?php echo $new ?>" placeholder="Enter the No"    / ></td>
</tr>
<?php
 } 
 ?>
...
<?php   
        ...
        include "connect.php";
        $i = mysql_query("insert into xray values('".$_POST['no']."','".trim($_POST['id'])."','".trim($_POST['did'])."','".trim($_POST['time'])."','".trim($_POST['pay'])."','".trim($_POST['parts'])."','".trim($_POST['date'])."')");
        if($i==true){

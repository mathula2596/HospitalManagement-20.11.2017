...
<td><input type="text" name="pid" size="30"  value=""  id="pid1"    / ></td>
</tr>
...
<td><select name="type" onChange="getId(this.value)">
<option selected hidden="">Choose the Doctor Type</option>
<?php
...
<script>
	...
	<?php   
        ...
        include "connect.php";
        $i = mysql_query("insert into appointment values('".$_POST['id']."','".trim($_POST['pid'])."','".trim($_POST['reason'])."','".trim($_POST['type'])."','".trim($_POST['doctor'])."','".trim($_POST['date'])."','".trim($_POST['time'])."')");
        if($i==true){

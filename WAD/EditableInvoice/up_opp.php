...
<?php
			include "connect.php";
			$i = "select * from theater where o_no='$_GET[o_no]'";
			$h = mysql_query($i);
			while($tr=mysql_fetch_array($h))
			{?>	
				<tr>
<td><label> Operation No : </label></td>
<td><input type="text" name="o_no" size="30" placeholder="Enter the No" readonly  value="<?php echo $tr[0]; ?>" / ></td>
</tr>
...
<option hidden selected><?php echo $tr[3]; ?></option>
...
<td><textarea name="reason" rows="5" cols="20" required> <?php echo $tr[4]; ?></textarea></td>
</tr>
...
<tr><td>
<label> Date : </label></td>
<td><input type="date" name="date" size="30" placeholder="Enter the date" required value="<?php echo $tr[5]; ?>" / ></td></tr>
<tr>
<tr></tr><tr></tr>
<td><label> Time : </label></td>
<td><input type="time" name="time" size="30" placeholder="Enter the time" required value="<?php echo $tr[6]; ?>" / ></td>
</tr>

<tr></tr><tr></tr>
<td><label> Payment : </label></td>
<td><input type="number" name="pay" size="30" placeholder="Enter the payment" required value="<?php echo $tr[7]; ?>" / ></td>
</tr>
...
<?php   
        ...
        include "connect.php";
        $i = mysql_query("UPDATE  theater SET  p_id='".$_POST['id']."',d_id='".$_POST['did']."',parts='".$_POST['parts']."',Reason='".$_POST['reason']."',	date='".$_POST['date']."',time='".$_POST['time']."',payment='".$_POST['pay']."'WHERE o_no='".$_POST[o_no]."'");
        if($i==true){
			   echo '<script language="javascript">';
		    echo 'alert("Successfully Updated...");';
			   echo '</script>';

        echo '<META HTTP-EQUIV="Refresh" Content="0; URL=view_opp.php">';
        }

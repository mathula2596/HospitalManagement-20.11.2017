...
<!-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------->
<br><br><h1 style="font-family:Georgia, 'Times New Roman', Times, serif; font-style:normal; text-align:center; color:#51b8f1;"> Receptionist Details</h1><br>
...
<form action="" name="myform" method="post" enctype="multipart/form-data">
  ...
  <?php
  			include "connect.php";
  			$i = "select * from receiptionist where ID='$_GET[id]'";
  			$h = mysql_query($i);
...
<td><input type="text" name="id" size="30" placeholder="Enter the ID" readonly  value="<?php echo $tr[0]; ?>" / ></td>
</tr>
...
<td><input type="text" name="f_name" size="30"  value="<?php echo $tr[1]; ?>" placeholder="Enter the first name" required  / ></td>
</tr>
...
<td><input type="NUMBER" name="p_number1" size="30"  value="<?php echo $tr[3]; ?>" maxlength="10"  pattern=".{9,}" placeholder="Enter the contact number" required /></td></tr>
<tr>
...
<td><input type="text" name="username" readonly size="30"  value="<?php echo $tr[10]; ?>"   /></td></tr>

<tr></tr><tr></tr><tr></tr>

<tr><td>
<label> Password : </label></td>
<td><input type="password" name="password" size="30"  value="<?php echo $tr[11]; ?>" placeholder="Enter the Password" required  /></td></tr>
...
<?php
    ...
    //  $i = mysql_query("UPDATE  appointment SET  P_ID='".$_POST['pid']."',Reason='".$_POST['reason']."',Doctor_Type='".$_POST['type1']."',Doctor_ID='".$_POST['doctor']."',	Date='".$_POST['date']."',Time='".$_POST['time']."'WHERE A_ID='".$_POST[A_ID]."'");
        ...
        $i = mysql_query("UPDATE receiptionist SET F_Name='".$_POST['f_name']."',L_Name='".$_POST['l_name']."',Phone='".$_POST['p_number1']."',DOB='".$_POST['date']."',Age='".$_POST['age']."',Address='".$_POST['address']."',Gender='".$_POST['gender1']."',	Email='".$_POST['mail']."',DOJ='".$_POST['join']."',username='".$_POST['username']."',password='".$_POST['password']."'WHERE ID='".$_POST[id]."'");
		$x=mysql_query("UPDATE  login SET username='".$_POST['username']."',password='".$_POST['password']."'WHERE 	ID='".$_POST[id]."'");
        if($i==true && $x==true){
			   echo '<script language="javascript">';
		    echo 'alert("Successfully Updated...");';
			   echo '</script>';

        echo '<META HTTP-EQUIV="Refresh" Content="0; URL=view_rec.php">';
